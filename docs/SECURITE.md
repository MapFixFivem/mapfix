...
- **`CONFIG.webhookUrl` est une URL de webhook Discord écrite en clair dans le code, choix assumé.** C'est un vrai
  secret : quiconque la lit (`Ctrl+U`, ou directement sur GitHub) peut poster de faux messages dans ce salon — un
  webhook ne permet que d'écrire, jamais de lire les autres salons ni les membres. C'est nécessaire pour que la
  notification parte vraiment quand un **client** utilise le site (pas seulement l'admin) : un réglage purement
  local (le triple-clic sur le logo, stocké en `localStorage`) ne s'applique qu'au navigateur qui l'a saisi, jamais à
  celui des clients — pour eux, il faut que le code lui-même connaisse déjà l'URL. Un fichier commis reste par
  ailleurs lisible pour toujours dans l'historique GitHub, même après l'avoir « retiré » d'un commit suivant, et des
  robots scannent en continu les dépôts publics à la recherche de ce motif (GitHub fait tourner un partenariat de
  révocation automatique avec Discord sur les secrets détectés dans les dépôts publics, sans garantie de délai). Si
  ça arrive, ou en cas de doute, supprime le webhook dans Discord et recrées-en un autre : l'ancienne URL devient
  inerte immédiatement, et le nouveau se règle en une ligne dans `js/config.js`. Pour une protection qui n'expose
  jamais l'URL, même en clair (l'URL vivrait sur un petit serveur au lieu du navigateur), il faudrait un relais (ex.
  un Cloudflare Worker) — hors du périmètre « site 100 % statique » de ce projet, mais ajoutable si besoin.
